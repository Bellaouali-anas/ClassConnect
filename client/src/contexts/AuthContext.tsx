...
interface AuthContextType {
  ...
  signUp: (email: string, password: string, userData: { firstName: string; lastName: string; userType: string }) => Promise<{ error: AuthError | null }>;
    ...
    const { data: { subscription } } = supabase.auth.onAuthStateChange(
      async (event, session) => {
        ...
        if (session?.user && session.user.app_metadata?.provider === 'email') {
          ...
          setIsPasswordResetSession(false);
        }
